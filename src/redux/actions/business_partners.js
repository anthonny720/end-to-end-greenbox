import {
    ...
    GET_FULL_CLIENTS_FAIL,
    GET_FULL_CLIENTS_SUCCESS,
    GET_PROVIDERS_FAIL,
...
export const get_full_clients = () => async dispatch => {
    const config = {
        headers: {
            'Authorization': `JWT ${localStorage.getItem('access')}`,
            'Accept': 'application/json'
    ...
    try {
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/business-partners/full-clients`, config);
        if (res.status === 200) {
            dispatch({
                type: GET_FULL_CLIENTS_SUCCESS,
                payload: res.data
            });
        } else {
            dispatch({
                type: GET_FULL_CLIENTS_FAIL
            });
        }
    } catch (err) {
        dispatch({
...
export const get_business_maquila = () => async dispatch => {
    ...
    try {
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/business-partners/business-maquila`, config);
        if (res.status === 200) {
            dispatch({
