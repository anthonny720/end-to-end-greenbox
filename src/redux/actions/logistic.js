import {
    ...
    GET_DATA_PROGRAM_SUCCESS, GET_FULL_PACKING_LIST_FAIL, GET_FULL_PACKING_LIST_SUCCESS,
...
export const get_full_packing_list = () => async dispatch => {
    const config = {
        headers: {
            'Authorization': `JWT ${localStorage.getItem('access')}`, 'Accept': 'application/json'
        }
    ...
    try {
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/logistic/full-packing-list`, config);
        if (res.status === 200) {
            dispatch({
                type: GET_FULL_PACKING_LIST_SUCCESS, payload: res.data
            });
        } else {
            dispatch({
                type: GET_FULL_PACKING_LIST_FAIL
            });
        }
    } catch (err) {
        dispatch({
