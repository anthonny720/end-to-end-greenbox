import {
    ...
    GET_LOTS_SUCCESS,
    UPDATE_I_LOT_FAIL,
    UPDATE_I_LOT_SUCCESS, UPDATE_LOT_FAIL, UPDATE_LOT_SUCCESS
} from "./types";
...
export const update_lot = (data,lot) => async dispatch => {
    const config = {
        headers: {
            'Authorization': `JWT ${localStorage.getItem('access')}`, 'Accept': 'application/json'
        }
    ...
    try {
        const res = await axios.patch(`${process.env.REACT_APP_API_URL}/api/raw-material/lot/${lot}`,data, config);
        if (res.status === 200) {
            dispatch({
