...
import FormTC from "../../components/rawmaterial/FormTC";
import FormInvoice from "../../components/rawmaterial/FormInvoice";
...
const DetailLot = () => {
    ...
    const handleOpenModalUpdateInfo = (row) => {
        setTitle(`Pallet ${row.number}`)
        setIsOpen(true)
        setContent(<Form pallets={pallets} locations={locations} data={row} entry={lot} lot={info?.id}
                         close={openModal}/>)
    }
    ...
    const handleSubmitTC = () => {
        setTitle(`Subir TC`)
        setIsOpen(true)
        setContent(<FormTC lot={lot}
                           close={openModal}/>)
    }
    const handleSubmitInvoice = () => {
        setTitle(`Subir factura`)
        setIsOpen(true)
        setContent(<FormInvoice lot={lot}
                                close={openModal}/>)
    }
            ...
            <div className={"lg:max-w-2xl sm:max-w-lg max-w-xs "}>
                ...
                <div className={"inline-flex"}>
                    ...
                    {info?.closed ? <>
                        ...
                        />
                        <ReactToPrint
                            documentTitle={`INF. N° ${info?.lot.substring(info?.lot.length - 3)} - RECP MAT PRIM ${info?.category_name.toUpperCase()} ${info?.provider_name.toUpperCase()}`}
                            trigger={() => <button
                ...
                {output.data !== null &&
                    <ReactToPrint documentTitle={"Vale de salida " + lot + " " + new Date().toLocaleDateString()}
                                  trigger={() => <button
