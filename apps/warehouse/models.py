...
class Reception(models.Model):
    ...
    def full_clients(self):
        if self.client:
            if self.client.count() == 1:
                client = self.client.first().name
            else:
                client = ", ".join(str(c.name) for c in self.client.all())
                client = "mixto " + str(client)
            return client
        else:
            return " "
    ...
    def short_name(self):
        if self.client.count() != 1:
            return self.type + " " + str(self.number) + " mixto " + self.denomination
        else:
            return self.type + " " + str(self.number) + " " + self.client.first().name + " " + self.denomination
...
def set_packing(sender, instance, **kwargs):
    try:
        p, created = PackingList.objects.get_or_create(reception=instance, slug=instance.slug)
        if created:
            p.save()
        else:
            p.objects.update(reception=instance, slug=instance.slug)
    except:
        pass
...
class PackingList(models.Model):
    ...
    def full_name(self):
        if self.destine:
            return str(self.reception.full_name()) + " " + str(self.destine)
        else:
            return str(self.reception.full_name())

    def short_name(self):
        if self.destine:
            return str(self.reception.short_name()) + " " + str(self.destine)
        else:
            return str(self.reception.short_name())
