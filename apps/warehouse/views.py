...
class PackingListView(APIView):
    def get(self, request, *args, **kwargs):
        try:
            ...
            return Response({'error': 'No se encontraron registros de PackingList'},
                            status=status.HTTP_404_NOT_FOUND)


class FullPackingListView(APIView):
    def get(self, request, *args, **kwargs):
        try:
            if PackingList.objects.all().exists():
                packing_list = PackingList.objects.all()[:50]
                serializer = IPackingListSerializer(packing_list, many=True)
                return Response({'result': serializer.data, 'status': status.HTTP_200_OK})
            else:
                raise ValueError('No hay registros de packing list')
        except Exception as e:
            return Response({'error': 'No se encontraron registros de PackingList', 'detail': str(e)},
                            status=status.HTTP_404_NOT_FOUND)
