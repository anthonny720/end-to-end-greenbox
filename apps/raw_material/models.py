...
def custom_tc_file_path(instance, filename):
    ext = filename.split('.')[-1]
    filename = "%s.%s" % (uuid.uuid4(), ext)
    return '/'.join(['MATERIA_PRIMA', str(date.today().year), instance.category.name, instance.lot, filename])


def custom_invoice_file_path(instance, filename):
    ext = filename.split('.')[-1]
    filename = "%s.%s" % (uuid.uuid4(), ext)
    return '/'.join(['MATERIA_PRIMA', str(date.today().year), instance.category.name, instance.lot, filename])
...
class Lot(models.Model):
    ...
    category = models.ForeignKey(Fruits, on_delete=models.PROTECT, related_name="fruit_entry", verbose_name="Categoria")
    ...
    def get_year(self):
        try:
            return self.entryDate.strftime('%Y')
        except:
            return "0"

    def get_month(self):
        try:
            return self.entryDate.strftime('%B')
        except:
            return "0"

    def get_week(self):
        try:
            return self.entryDate.isocalendar()[1]
        except:
            return 0
...
def pre_save_image(sender, instance, *args, **kwargs):
    ...
    try:
        old_img = instance.__class__.objects.get(id=instance.id).invoice.path
        try:
            new_img = instance.invoice.path
        except:
            new_img = None
        if new_img != old_img:
            import os
    ...
    try:
        old_img = instance.__class__.objects.get(id=instance.id).tc.path
        try:
            new_img = instance.tc.path
        except:
            new_img = None
        if new_img != old_img:
            import os
